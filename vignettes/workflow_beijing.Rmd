...
abimo@data$REGENJA <- mean(climate_data$sum_annual.p)
abimo@data$REGENSO <- mean(climate_data$sum_summer.p)

kwb.abimo::abimo_xml_evap(file_in = paths$abimo_config_original,
                          file_out = paths$abimo_config_modified,
                          line_evap = c(42:46, 49),
                          evap_annual = mean(climate_data$sum_annual.etp),
                          evap_summer = mean(climate_data$sum_summer.etp)
)

# to do: improve kwb.abimo by using xml2 (as approach above currently misses 
# one line for etp replacement !!!)
# tmp <- xml2::read_xml(paths$abimo_config_original)
# 
# tmp <- xml2::read_xml(paths$abimo_config_original) %>%  xml2::as_list()
# 
# section_names <- as.character(sapply(tmp$sections, function(x) attr(x, "name")))
# 
# attr(tmp$sections[which(section_names == "PotentielleVerdunstung")], "etp")
# 
# xml2::xml_find_all(tmp, "//*[name = 'PotentielleVerdunstung']")
# baz <- xml2::xml_find_all(tmp, ".//section")
# rvest::xml_nodes(tmp, "section") %>% 
#   xml2::xml_attrs("name")
